...
import { fetchDay, fetchMonthSnapshot, saveNotes, toggleCheck } from './actions';
import { fetchEventsByDate } from './actions';  

type DayMap = Record<string, { date: string; checks: Record<string, boolean>; notes: string | null }>;
...
function monthTitle(y: number, m0: number) {
  return new Intl.DateTimeFormat('en', { month: 'long', year: 'numeric' }).format(new Date(Date.UTC(y, m0, 1)));
}
...
export default function DailyTrackerClient(props: {
  ...
  const grid = useMemo(() => {
    ...
    const cells: (string | null)[] = Array(firstDow).fill(null).concat(
      Array.from({ length: total }, (_, i) => dateKey(new Date(Date.UTC(year, m0, i + 1))))
    );
    while (cells.length % 7 !== 0) cells.push(null);
    return cells;
  }, [year, m0]);

  // load selected day on mount / when changed
  useEffect(() => {
    startTransition(async () => {
      const d = await fetchDay(selected);
      setNotes(d?.notes ?? '');
      const events = await fetchEventsByDate(selected);
      setEvents(events);
    });
  }, [selected]);
  ...
  async function changeMonth(delta: number) {
    startTransition(async () => {
      ...
      const ym = await fetchMonthSnapshot(dt.getUTCFullYear(), dt.getUTCMonth());
  ...
  const selectedDay = monthMap[selected] ?? { date: selected, checks: {}, notes: null };
    ...
    <div className="max-w-6xl mx-auto px-4 py-8">
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-2">
          <button onClick={() => changeMonth(-1)} className="px-3 py-2 rounded-xl bg-slate-800 hover:bg-slate-700">◀</button>
          <div className="min-w-[12rem] text-center text-slate-300">{monthTitle(year, m0)}</div>
          <button onClick={() => changeMonth(1)} className="px-3 py-2 rounded-xl bg-slate-800 hover:bg-slate-700">▶</button>
        </div>
      </div>
        ...
        <div className="grid grid-cols-7 gap-2">
          {grid.map((dk, idx) => dk ? (
            <button
              ...
              className={[
                'aspect-square rounded-xl border text-sm',
                'transition',
                selected === dk
            ...
            >
              ...
              {monthMap[dk] && <div className="mt-1 mx-auto h-1.5 w-1.5 rounded-full bg-emerald-400" />}
            </button>
          ) : <div key={`e-${idx}`} />)}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div className="lg:col-span-2 rounded-2xl border border-slate-800/60 bg-slate-950/40 p-5">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {DAILY_TEMPLATE.map((sec) => (
              <div key={sec.title} className="rounded-xl border border-slate-800 bg-slate-900/50 p-3">
        ...
        <div className="rounded-2xl border border-slate-800/60 bg-slate-950/40 p-5">
          <div className="flex items-center justify-between mb-3">
            ...
            <button
              onClick={() => startTransition(async () => { await saveNotes(selected, notes); })}
